    ...
    methods: {
        ...
        getXandYFromEvent(event) {
            ...
            if (targetTouches) {
                ...
                const touchElement =
                    element &&
                    (element.closest(".nestable-item-content") ||
                        element.closest(".nestable-list-empty"));
        ...
        onMouseMove(event) {
            ...
            const elDragLayer = document.querySelector(
                ".nestable-" + this.group + " .nestable-drag-layer"
            );
            if (!elDragLayer) return;
            const { top: dragLayerTop, left: dragLayerLeft } =
        ...
        tryIncreaseDepth(dragItem) {
            ...
            if (itemIndex > 0 && newDepth <= this.maxDepth) {
                ...
                if (
                    ...
                    (!prevSibling[this.childrenProp].length ||
        ...
        isCollapsed(item) {
            return !!(
                (this.collapsedGroups.indexOf(item[this.keyProp]) > -1) ^
                this.collapsed
            );
        },
